...
6. **`agy-insights`**
   - **설명**: Antigravity 대화 로그와 git 히스토리를 분석해 인터랙티브 HTML 세션 인사이트 대시보드 생성
   - **포함 자료**: 리포트 생성 스크립트(`scripts/generate_insights.py`), 평가셋(`evals/evals.json`)
   - **참고**: Claude Code에는 내장 `/insights`가 있고 개인 스킬이 내장보다 우선하므로, 이름을 `agy-insights`로 비켜 두었습니다. Claude Code 세션 분석은 내장 `/insights`, 토큰·비용은 `/usage`를 씁니다.
7. **`compact`** *(AGY 전용)*
...
- **`statusline`**
  ...
  - 한 번 설치하면 끝 자세한 내용은 [`statusline/README.md`](statusline/README.md)
...
ln -s ~/projects/my-agent-skills/agy-insights ~/.gemini/config/skills/
...
ln -s ~/projects/my-agent-skills/agy-insights ~/.claude/skills/
```
> `compact`은 Claude Code 내장 `/compact` 명령과 이름이 겹치므로 AGY에만 링크합니다.
> `agy-insights`는 내장 `/insights`를 가리지 않도록 이름을 바꿔 두어 양쪽 모두 링크합니다.
...
- `/agy-insights` 또는 `agy-insights 스킬로 Antigravity 세션 리포트 만들어줘`
...
- *"이번 주 Antigravity 세션 활동 분석해줘"* -> `agy-insights` 자동 트리거
